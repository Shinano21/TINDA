...
class PurchaseReportView(LoginRequiredMixin, PermissionRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_items_purchased = queryset.aggregate(total=Sum('qty'))['total']
        total_costs = queryset.aggregate(total=Sum('total'))['total']
        # aggregate returns None when there are no records, convert to 0
        if total_items_purchased is None:
        ...
        context['total_items_purchased'] = total_items_purchased
        context['total_costs'] = self.format_total(total_costs)

        return context

    def format_total(self, total):
        """Return formatted string for a numeric total.

        Accepts None or numeric values.  If ``total`` is ``None`` we treat
        it as zero to avoid ``TypeError`` when formatting.  The returned
        string uses comma grouping and two decimal places, e.g. ``"1,234.00"``.
        """
        if total is None:
            total = 0
        return f"{total:,.2f}"
